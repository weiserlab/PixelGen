...
//     for (size_t i = 0; i < ARTEMIS_TASK_ID_ENV; i++) {
...
void artemis_scheduler_run(void)
{
    ...
    // uint64_t current_us;
    // uint64_t elapsed_us;
    task = artemis_task_get(0); //lux
    task->initialize(); //initiazlie i2c
    task->run(); //run lux first
    task = artemis_task_get(1);
    task->initialize(); //initiazlie bme parameter
    task->run(); //run bme
    while (1)
    {
