// optimized Camera only code for PixelGen
...
#define BUTTON_GPIO_PIN 16                // Camera Image Click Button Pin 1
#define BUTTON_GND_GPIO_PIN 26            // Camera Image Click Button - GND (Active LOW)
#define CAMERA_EN_PIN 32                  // Camera Enable Pin

//*****************************************************************************
// BUTTON0 pin configuration settings on PIN 16
//*****************************************************************************
// Set up the configuration for BUTTON0.
const am_hal_gpio_pincfg_t button_gpio =
{
    .uFuncSel = 3,
...
int main(void)
{
    ...
    // Setting clock to 96 MHz
    am_hal_clkgen_control(AM_HAL_CLKGEN_CONTROL_SYSCLK_MAX, 0);

    // ui32HFRC = am_hal_clkgen_sysclk_get();

    // am_util_stdio_printf("=================================");
    // am_util_stdio_printf("Clock Source %d\n", ui32HFRC);
    // am_util_stdio_printf("=================================");
    //
...
// #if defined(AM_BSP_NUM_BUTTONS)
    ...
    // sPinCfg.ePull = AM_HAL_PIN_11_PDMDATA;
    // am_hal_gpio_pinconfig(BUTTON_GPIO_PIN, g_AM_HAL_GPIO_INPUT_PULLUP);
    am_hal_gpio_pinconfig(BUTTON_GPIO_PIN, g_AM_HAL_GPIO_INPUT);

    const am_hal_gpio_pincfg_t PULLDOWN_PIN_CFG =
    {
        .uFuncSel       = 3,
        // .eGPOutcfg      = AM_HAL_GPIO_PIN_OUTCFG_DISABLE,
        // .eGPInput       = AM_HAL_GPIO_PIN_INPUT_ENABLE,
    ...
    // am_hal_gpio_pinconfig(BUTTON_GND_GPIO_PIN, PULLDOWN_PIN_CFG);
    // am_hal_gpio_output_clear(BUTTON_GND_GPIO_PIN);
    am_hal_gpio_pinconfig(BUTTON_GND_GPIO_PIN, g_AM_HAL_GPIO_INPUT_PULLUP);
    // am_hal_gpio_state_write(BUTTON_GND_GPIO_PIN, AM_HAL_GPIO_OUTPUT_CLEAR);
    am_hal_gpio_pinconfig(CAMERA_EN_PIN, g_AM_HAL_GPIO_INPUT_PULLUP);
...
// #endif
 ...
 ///////////////////////////////////////////////////
    ...
    // while(1){
    //     if(am_hal_gpio_input_read(BUTTON_GPIO_PIN) == 1){
    //         am_util_stdio_printf("life jhingalala");
    //     }
    //     else{
    //         am_util_stdio_printf("life sad");
    //     }
    // }
    // while(1)
    // {
    ...
    while (1)
    {
        // am_util_stdio_printf("Cam OUT\n");
        am_hal_gpio_state_read(BUTTON_GPIO_PIN, AM_HAL_GPIO_INPUT_READ, &ui32Value);
        // am_util_stdio_printf("Return Val = %d, GPIO State = %d\n",ui32Ret, ui32Value);
        if(ui32Value){

            // am_util_stdio_printf("Cam IN\n");
            ...
            // am_util_stdio_printf("Deep sleep");
            am_hal_sysctrl_sleep(AM_HAL_SYSCTRL_SLEEP_NORMAL);
            // am_util_delay_ms(1000);
        }
    }
